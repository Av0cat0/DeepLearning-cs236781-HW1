...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        y_pred = None
        # ====== YOUR CODE: ======
        y_pred = np.dot(self.weights_,(X.T))
        # ========================
    ...
    def fit(self, X, y):
        ...
        reg =  self.reg_lambda * X.shape[0] * np.identity(X.shape[1])
        reg[0,0] = 1
        w_opt = np.dot(np.dot(np.linalg.inv(np.dot(X.T , X) + reg) , X.T) , y)
        # ========================
...
class BiasTrickTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: np.ndarray):
        ...
        xb = None
        # ====== YOUR CODE: ======
        #using hstack and np.ones for biastrick
...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_transformed = None
        # ====== YOUR CODE: ======
        X_transformed = PolynomialFeatures(self.degree).fit_transform(X[:,1:])
        # ========================
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    corr_target = df.corr()[target_feature]
    corr_target_row = corr_target.drop([target_feature])
    top_n = corr_target_row.abs().nlargest(n)
    top_n_features = list(top_n.index)
    top_n_corr = corr_target_row[top_n.index].values
    # ========================
...
def mse_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    mse = np.mean((y - y_pred).T@((y - y_pred)))/ (y.shape[0])
    # for avg result divide by the number of numbers on matrix (N^2)
    # ========================
    return mse
...
def r2_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    r2 = 1 - (np.sum((y - y_pred).T@((y - y_pred))))/(np.sum((y - y.mean()).T@(y - y.mean())))
    # ========================
    return r2
...
def cv_best_hyperparams(
    ...
    learning_model = sklearn.model_selection.GridSearchCV(model, params, cv=k_folds, scoring='r2')
    learning_model.fit(X, y)
    best_params =  learning_model.best_estimator_.get_params()
    # ========================
