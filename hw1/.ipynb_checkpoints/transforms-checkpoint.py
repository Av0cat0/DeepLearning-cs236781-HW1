...
class BiasTrick(object):
    ...
    def __call__(self, x: torch.Tensor):
        ...
        # Make tensor of ones in the number of dimensions with the right dtype
        ones_to_bias = torch.ones((list(x.shape[:-1]))+[1], dtype = x.dtype) 
        
        # Cat it so 1 will be at the beginning 
        return torch.cat((ones_to_bias, x), x.dim()-1)
