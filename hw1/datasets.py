...
def random_labelled_image(
    ...
    # Creating random tensors (boundaries and dims) according to the provided parameters
    image=torch.randint(low, high,(shape[0],shape[1],shape[2])) 
...
def torch_temporary_seed(seed: int):
    ...
    try:
        # Creating manual seed while using the yield keyword
        # so the parent process will continue executed 
        torch.random.manual_seed(seed)
        yield
    finally:
        # Returing to the previous state
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        # Using the temporary seed within a context so it won't change
        # the current rng and will apply only to the interior code.
        with torch_temporary_seed(index):  
            
            # Checking if index is out of range
            if (index > self.num_samples-1 or index <0):
                raise ValueError
                
            # Creating the labelled image with in the temporary seed context
            # therefore managing the randomness of the tests
            X, y = random_labelled_image(self.image_dim, self.num_classes, low=0, high=255)
            return (X, y)
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        # Infinite loop resulting in infinite samples
        while True:
            
            # Creating a new sample and yielding it so the function will never ends
            # and a new sample will be returned each time __iter__() is called
            yield random_labelled_image(self.image_dim, self.num_classes, low=0, high=255)
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Checking that index is not out of range to return an existing object
        if(index > self.subset_len-1 or index <0 ):
            raise IndexError
        
        # Returns the image and label tensors with the index of index+offset 
